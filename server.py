from sqlite3 import Connection as SQLite3Conection
...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///databasefile.db"
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = 0

# Configure sqlite3 to enforce foreign key contraints
@event.listens_for(Engine, "connect")
def _set_sqlite_pragma(dbapi_connection, connection_record):
    if isinstance(dbapi_connection, SQLite3Conection):
...
def get_all_users_ascending():
    # That's enough actually because it returns data in that order already
    users = User.query.all()
    # And this code is redundant - we don't have to use linked list here
    all_users_ll = linked_list.LinkedList()
